...
enum class DotImplementationStrategy {
  ...
  // The dot operation is lowered into a call into a routine
  // implemented in an external bitcode file.
  kExternalBitcode,
};
...
Status DotOpEmitter::Emit() {
  ...
  switch (GetDotImplementationStrategy(hlo_module_config_, dot_info_,
    ...
    case DotImplementationStrategy::kExternalBitcode:
...
DotImplementationStrategy GetDotImplementationStrategy(
  ...
  if (IsAlignedGemm(dot_info, target_machine_features)) {
    ...
    if (config.use_external_bitcode())
	...
	return DotImplementationStrategy::kExternalBitcode;
